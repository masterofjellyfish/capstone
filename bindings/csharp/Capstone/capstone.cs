...
namespace Capstone
{
    ...
    internal class CsStatus
    {
        ...
        internal CsStatus(ErrorCode capstoneError)
        {
            this._err = capstoneError;

            switch (capstoneError)
            {
                case ErrorCode.Mem:
                    this.message = "Out of memory (CsError)";
                    break;
                case ErrorCode.Arch:
                    this.message = "Invalid architecture (CsError)";
                    break;
                case ErrorCode.Handle:
                    this.message = "Invalid handle (CsError)";
                    break;
                case ErrorCode.Csh:
                    this.message = "Invalid csh (CsError)";
                    break;
                case ErrorCode.Mode:
                    this.message = "Invalid mode (CsError)";
                    break;
                case ErrorCode.Option:
                    this.message = "Invalid option (CsError)";
                    break;
                default:
                    this.message = "";
                    break;
        ...
        public static bool operator ==(CsStatus a, ErrorCode b)
        {
            return a._err == b;
        }
        ...
        public static implicit operator CsStatus(ErrorCode err)
        {
            return new CsStatus(err);
        }
    ...
    public class CapstoneException : Exception
    {
        ...
        internal CapstoneException(CsStatus message) : base(message.ToString()) { }
    ...
    internal static class NativeMethods
    {
        ...
        [DllImport("libcapstone.so", CallingConvention = CallingConvention.Cdecl)]
        internal static extern UIntPtr cs_disasm_dyn(UIntPtr handle, byte[] code,
                UIntPtr code_size, ulong address, UIntPtr count, ref IntPtr insn);
        [DllImport("libcapstone.so", CallingConvention = CallingConvention.Cdecl)]
        [return: MarshalAs(UnmanagedType.Bool)]
        internal static extern bool cs_free(IntPtr insn);
        [DllImport("libcapstone.so", CallingConvention = CallingConvention.Cdecl)]
        internal static extern ErrorCode cs_close(UIntPtr handle);
        [DllImport("libcapstone.so", CallingConvention = CallingConvention.Cdecl)]
        internal static extern ErrorCode cs_errno(UIntPtr handle);
        [DllImport("libcapstone.so", CallingConvention = CallingConvention.Cdecl)]
        internal static extern void cs_version(ref int major, ref int minor);
        [DllImport("libcapstone.so", CallingConvention = CallingConvention.Cdecl)]
    ...
    public class Capstone : IDisposable
    {
        ...
        internal struct _Insn
        {
            ...
            public uint regs_read_count;
            [MarshalAs(UnmanagedType.ByValArray, SizeConst = 32)]
            public int[] regs_write;
            public uint regs_write_count;
        ...
        public Version Version
        {
            ...
            get
            {
                ...
                NativeMethods.cs_version(ref major, ref minor);
        ...
        public Capstone(Architecture arch, Mode mode)
        {
            ...
            string libPath = Path.Combine(Path.GetDirectoryName(typeof(Capstone._Insn).Assembly.Location),
        ...
        public Instruction[] Disassemble(byte[] code, ulong addr, UIntPtr count)
        {
            ...
            ulong numInst = (ulong) NativeMethods.cs_disasm_dyn(this._handle, code, (UIntPtr)(uint)code.Length, addr, count, ref ptr);
            ...
            if (numInst > 0)
            {
                ...
                for (ulong j = 0; j < numInst; j++)
                {
                    ...
                    IntPtr archPtr = new IntPtr(data.ToInt64() + (Int64)(Marshal.SizeOf(typeof(_Insn))));
                    ...
                    if (_insn.regs_read_count > 0)
                    {
                        insns[j].Regs_read = new Object[_insn.regs_read_count];
                        Array.Copy(_insn.regs_read, insns[j].Regs_read, _insn.regs_read_count);
                    }

                    if (_insn.regs_write_count > 0)
                    {
                        insns[j].Regs_write = new Object[_insn.regs_write_count];
                        Array.Copy(_insn.regs_write, insns[j].Regs_write, _insn.regs_write_count);
                    }

                    if (_insn.groups_count > 0)
                    {
                        insns[j].Groups = new Object[_insn.groups_count];
                        Array.Copy(_insn.groups, insns[j].Groups, _insn.groups_count);
                    }
                    ...
                    switch (_arch)
                    {
                        case Architecture.Arm:
                            ...
                            if (_insn.regs_read_count > 0) { for (int k = 0; k < insns[j].Regs_read.Length; k++) { insns[j].Regs_read[k] = (Arm.REG)insns[j].Regs_read[k]; }; }
                            if (_insn.regs_write_count > 0) { for (int k = 0; k < insns[j].Regs_write.Length; k++) { insns[j].Regs_write[k] = (Arm.REG)insns[j].Regs_write[k]; }; }
                            if (_insn.groups_count > 0) { for (int k = 0; k < insns[j].Groups.Length; k++) { insns[j].Groups[k] = (Arm.GRP)insns[j].Groups[k]; }; }
                            break;
                        case Architecture.Arm64:
                            insns[j].Id = (Arm64.INSN) _insn.id;
                            insns[j].Arch = Marshal.PtrToStructure(archPtr, typeof(Arm64.CsArm64));
                            if (_insn.regs_read_count > 0) { for (int k = 0; k < insns[j].Regs_read.Length; k++) { insns[j].Regs_read[k] = (Arm64.REG)insns[j].Regs_read[k]; }; }
                            if (_insn.regs_write_count > 0) { for (int k = 0; k < insns[j].Regs_write.Length; k++) { insns[j].Regs_write[k] = (Arm64.REG)insns[j].Regs_write[k]; }; }
                            if (_insn.groups_count > 0) { for (int k = 0; k < insns[j].Groups.Length; k++) { insns[j].Groups[k] = (Arm64.GRP)insns[j].Groups[k]; }; }
                            break;
                        case Architecture.Mips:
                            insns[j].Id = (Mips.INSN) _insn.id;
                            insns[j].Arch = Marshal.PtrToStructure(archPtr, typeof(Mips.CsMips));
                            if (_insn.regs_read_count > 0) { for (int k = 0; k < insns[j].Regs_read.Length; k++) { insns[j].Regs_read[k] = (Mips.REG)insns[j].Regs_read[k]; }; }
                            if (_insn.regs_write_count > 0) { for (int k = 0; k < insns[j].Regs_write.Length; k++) { insns[j].Regs_write[k] = (Mips.REG)insns[j].Regs_write[k]; }; }
                            if (_insn.groups_count > 0) { for (int k = 0; k < insns[j].Groups.Length; k++) { insns[j].Groups[k] = (Mips.GRP)insns[j].Groups[k]; }; }
                                break;
                        case Architecture.X86:
                            insns[j].Id = (X86.INSN) _insn.id;
                            insns[j].Arch = Marshal.PtrToStructure(archPtr, typeof(X86.CsX86));
                            if (_insn.regs_read_count > 0) { for (int k = 0; k < insns[j].Regs_read.Length; k++) { insns[j].Regs_read[k] = (X86.REG)insns[j].Regs_read[k]; }; }
                            if (_insn.regs_write_count > 0) { for (int k = 0; k < insns[j].Regs_write.Length; k++) { insns[j].Regs_write[k] = (X86.REG)insns[j].Regs_write[k]; }; }
                            if (_insn.groups_count > 0) { for (int k = 0; k < insns[j].Groups.Length; k++) { insns[j].Groups[k] = (X86.GRP)insns[j].Groups[k]; }; }
                            break;
                    }
                }

                NativeMethods.cs_free(ptr);
